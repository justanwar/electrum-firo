...
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    ...
    FRESH_CLONE="$CONTRIB_APPIMAGE/fresh_clone/electrum_firo" && \
        rm -rf "$FRESH_CLONE" && \
        umask 0022 && \
...
docker run -it \
    ...
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrum_firo \
    --rm \
    --workdir /opt/electrum_firo/contrib/build-linux/appimage \
